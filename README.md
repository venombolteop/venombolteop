...
<img src="https://github-readme-activity-graph.vercel.app/graph?username=Venombolteop&theme=tokyo-night&hide_border=false" alt="Activity Graph" width="100%"/>

</div>

<br>

## 📈 GitHub Metrics — private contributions included

<div align="center">

<!-- Rich metric card generated by the "GitHub Metrics" workflow — aggregates
     private + public contributions, languages & achievements. Never shows repo
     names or details, only numbers. -->
...
<!--START_SECTION:snake-->
<img src="https://raw.githubusercontent.com/Venombolteop/Venombolteop/output/github-contribution-grid-snake.svg" alt="snake animation" />
<!--END_SECTION:snake-->
...
This profile is powered by four GitHub Actions workflows that keep every widget up to date automatically:
...
| [`metrics.yml`](.github/workflows/metrics.yml) | Generates the **rich metrics card** (private + public contributions, languages, achievements, lines of code) with [lowlighter/metrics](https://github.com/lowlighter/metrics) — aggregates only, **never shows repo names** | Every day + manual |
...
<details>
<summary><b>🔐 Want private contributions in the metrics card? (one-time setup)</b></summary>

The default `GITHUB_TOKEN` can only see **public** data. To include your **private** contributions (still no repo names or details — just numbers):

1. Go to **github.com → Settings → Developer settings → Personal access tokens → Tokens (classic)**
2. **Generate new token (classic)** → tick the `repo` scope → **Generate** → copy the token
3. Back on this repo: **Settings → Secrets and variables → Actions → New repository secret**
   - **Name:** `METRICS_TOKEN`
   - **Value:** paste the token
4. Run the **GitHub Metrics** workflow from the **Actions** tab once

The token is stored encrypted by GitHub and never appears in the README. If you skip this step, the metrics card simply renders with public data only.

> 🔒 Tip: the classic `repo`-scoped token is the standard for this. For maximum paranoia, use a **fine-grained** token limited to read access on your private repos and store it the same way.
